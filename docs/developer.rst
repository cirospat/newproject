...
* Bot Telegram COVID-19 Italia Aggiornamento: @covid19_dati_italia_bot 

* Protezione Civile Nazionale: \ |LINK7|\ 
...
:guilabel:`trovare la pagina esatta in un pdf online`: nel web è \ |LINK9|\  cioè dopo :class:`.pdf` digitare :class:`.#page=4` dove dopo :class:`=` si scrive il numero della pagina che si vuole venga visualizzata direttamente all'apertura del file ``pdf`` online
...
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|icone material design Google                     |\ |LINK10|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|webservice per la trascrizione testuale di audio |\ |LINK11|\                                                                               |
|                                                 |                                                                                          |
|con plugin per chrome                            |\ |LINK12|\                                                                               |
|                                                 |                                                                                          |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|\ |STYLE0|\  funzioni su Fogli Google spreadsheet|\ |LINK13|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|Markup Validation Service                        |\ |LINK14|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|Web accessibility Contrast Checker               |\ |LINK15|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|Web Developer Toolbar Chrome                     |\ |LINK16|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|Accessibility of a PDF Document                  |\ |LINK17|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
...
* \ |LINK20|\  my google maps

* \ |LINK21|\  (pagina FB con mappe su Palermo)

* \ |LINK22|\  (mappe varie di Ciro)

* \ |LINK23|\  (contenitore di oltre 100 mappe su Palermo, su Petrusino)

* \ |LINK24|\  (sito mappe di Palermo utili a turisti)

* \ |LINK25|\  (OMIRA su Umap cirospat) 

* \ |LINK26|\  elezioni2017_Palermo da Guenter Richter

* \ |LINK27|\  e relativo bot telegram #rifiutocivico

* \ |LINK28|\  mapillary cirospat

* \ |LINK29|\  Palermo su mapillary

* \ |LINK30|\  Palermo airbnb gen_2018 (Gianni Vitrano)
...
* \ |LINK31|\  (GBVitrano Palermo IGM 1:25000)

* \ |LINK32|\  carta tecnica comune Palermo

* \ |LINK33|\  (GBVitrano Omira 1:5000)

* \ |LINK34|\   PRG2025 (su Petrusino)

* \ |LINK35|\  (elenco mappe 2-3D di GBVitrano su Github SiciliaHub)

* \ |LINK36|\   (Hubsicilia - Omira Irta e Sas con OSM e RealVista su Gitub SiciliaHub, con metadati mappa su barra laterale)

* \ |LINK37|\  (variante general eal PRG 2004 con cerchio)

* \ |LINK38|\  (catasto centro storico 1887 con cerchio)

* \ |LINK39|\  (PRG scehma masima su PRG 2004 con cerchio)

* \ |LINK40|\    \ |LINK41|\    (Piano protezione civile Palermo)

* \ |LINK42|\   (stile petrusino)

* \ |LINK43|\   (stile petrusino, ultimo gen 2018) - hub di diverse mappe di Palermo a cura di GBVitrano iniziato gli ultimi giorni di dic. del 2017
...
\ |LINK44|\  (che preferisco) 

\ |LINK45|\  (intramontabile) 

\ |LINK46|\  (questo però è un plugin per wordpress) 
...
The \ |LINK50|\  function converts it to the required format for the Query formula by specifying a format of "yyyy-mm-dd":
...
* lista di tiles per sfondi   \ |LINK52|\  

* tiles vari da usare  \ |LINK53|\  

* satellite tile ESRI - World Imagery \ |LINK54|\  

* satellite tile Google \ |LINK55|\  

* tile con sfondo satellite da mapbox/satellite-v9 con API: \ |LINK56|\   (senza nomi strade)

* \ |LINK57|\  (con nomi strade)

* SERVIZIO ONLINE CREAZIONE DI TILES DA IMMAGINI  Mapwarper \ |LINK58|\  

* \ |LINK59|\  tile: https://api.mapbox.com/styles/v1/rplln/cjix2cv5l83yz2rmne9icvusx/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoicnBsbG4iLCJhIjoiY2ppeDI5ZTByMDVtMzN1cDRxZWlzamwzcyJ9.Yn6OjT0SStL2tffNloZrdg
...
video embedding = {{{\ |LINK60|\ }}}

link con testo = [[\ |LINK61|\ |testo del link]] 

immagine = {{\ |LINK62|\ |larghezza}} Immagine 

link a url con icona cliccabile = [[\ |LINK63|\ }}]]

ICONE MAPPE = \ |LINK64|\  

GOOGLEMAPS = <iframe width='100%' height='500' src="http....   "></iframe>

URL ENCODER E DECODER = \ |LINK65|\  

POST IMAGE DA LINKARE SU UMAP: \ |LINK66|\  

Icone mappe \ |LINK67|\  ccbysa
...
Video breve per alimentare in modo facile uMap da un foglio elettronico su Google Drive, by Andrea Borruso \ |LINK68|\ 

Tutorial per fare mappe alimentate da google spreadsheet = \ |LINK69|\  
...
.. _h4237131b4f2b2c466a1d35184e613840:

\ |LINK70|\  e \ |LINK71|\  UMAP AVVISI POLIZIA MUNICIPALE PA - VISUALIZZARE MAPPE CON INFO SU DATA ODIERNA
-----------------------------------------------------------------------------------------------------------
...
⇒ \ |LINK72|\  plug in come componente aggiuntivo da installare su spreadsheet di Google. Sfrutta API di Google e quindi i dati delle coordinate geografiche sono riusabili solo su mappe Google.
...
dove A2 è la colonna dove è contenuto l'indirizzo. \ |LINK73|\ .


|REPLACE6|

* \ |LINK74|\  con Openstreetmap tramite OnData

* \ |LINK75|\   con Openstreetmap

* \ |LINK76|\  con Openstreetmap

* \ |LINK77|\  con Openstreetmap

* \ |LINK78|\  con Googlemap

* \ |LINK79|\  by GBVitrano con API Google

* \ |LINK80|\  by GBVitrano con API Google

* \ |LINK81|\  con le API di Google

    * \ |LINK82|\  con API Google

* \ |LINK83|\  (google e bing) 

* \ |LINK84|\  con API di Mapquest

* \ |LINK85|\  API Mapquest per geocoding (cirospat - caneclaudia2volte)

* \ |LINK86|\  location helper in versione demo di Mapbox
...
* \ |LINK88|\  (mappa con grafici e dati su sensore mio)
...
just a simple guide for getting started with git. \ |LINK89|\ 
...
\ |LINK92|\  un articolo per capire un po

\ |LINK93|\  corso online strutturato bene per utenti con nozioni zero di intelligenza artificiale
...
* \ |LINK95|\  editor online sulla falsa riga di Google spreadsheet
...
* \ |LINK102|\  il mio genratore di feed RSS

* \ |LINK103|\  - \ |LINK104|\  per costruire Feed per gli eventi Feltrinelli di Palermo
...
* PDFTK Builder \ |LINK109|\  

* Online2PDF \ |LINK110|\  

* \ |LINK111|\  — servizio Web gratuito e che non richiede registrazioni. Permette di unire e modificare i file PDF in vari modi. Da notare che non consente di caricare file aventi un peso superiore ai 100 MB. Eventualmente, però, è possibile aggirare questa limitazione passando a uno dei piani a pagamento (con costi a partire da 6 euro/mese), i quali consentono di sbloccare anche altre funzionalità extra.

* \ |LINK112|\  — si tratta di un altro servizio Web gratuito e che non necessita di registrazione e permette di intervenire sui documenti PDF unendoli, oltre che convertendoli, dividendoli, alleggerendoli, sbloccandoli ecc. Non applica limitazioni per quel che concerne il peso massimo dei file.

* \ |LINK113|\  — è un’altra soluzione per unire i file PDF via Web e tramite la quale è eventualmente possibile apportare varie ulteriori modifiche ai documenti caricati. È gratis, non richiede registrazione e non pone limiti per l’upload.

\ |STYLE10|\ 

\ |STYLE11|\ . Si tratta, infatti, di un’app che consente di visualizzare i file PDF e intervenire su di essi in vari modi, andandoli a convertire, unire ecc. È gratuita e facile da usare. Per effettuarne il download sul tuo dispositivo, accedi alla \ |LINK114|\ 
...
:guilabel:`Queste ed altre indicazioni per Read the Docs si tovano anche a` \ |LINK116|\  
...
:download:\`Questo link è per il download <\ |LINK117|\ >\`.

si scrive così: ``:download:`Questo link è per il download <\ |LINK118|\ >```
