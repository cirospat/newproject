...
\ |LINK85|\  risorse utili per lavorare con le mappe
...
The \ |LINK87|\  function converts it to the required format for the Query formula by specifying a format of "yyyy-mm-dd":
...
* lista di tiles per sfondi   \ |LINK89|\  

* tiles vari da usare  \ |LINK90|\  

* satellite tile ESRI - World Imagery \ |LINK91|\  

* satellite tile Google \ |LINK92|\  

* tile con sfondo satellite da mapbox/satellite-v9 con API: \ |LINK93|\   (senza nomi strade)

* \ |LINK94|\  (con nomi strade)

* SERVIZIO ONLINE CREAZIONE DI TILES DA IMMAGINI  Mapwarper \ |LINK95|\  

* \ |LINK96|\  tile: https://api.mapbox.com/styles/v1/rplln/cjix2cv5l83yz2rmne9icvusx/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoicnBsbG4iLCJhIjoiY2ppeDI5ZTByMDVtMzN1cDRxZWlzamwzcyJ9.Yn6OjT0SStL2tffNloZrdg
...
video embedding = {{{\ |LINK97|\ }}}

link con testo = [[\ |LINK98|\ |testo del link]] 

immagine = {{\ |LINK99|\ |larghezza}} Immagine 

link a url con icona cliccabile = [[\ |LINK100|\ }}]]

ICONE MAPPE = \ |LINK101|\  

GOOGLEMAPS = <iframe width='100%' height='500' src="http....   "></iframe>

URL ENCODER E DECODER = \ |LINK102|\  

POST IMAGE DA LINKARE SU UMAP: \ |LINK103|\  

Icone mappe \ |LINK104|\  ccbysa
...
Video breve per alimentare in modo facile uMap da un foglio elettronico su Google Drive, by Andrea Borruso \ |LINK105|\ 

Tutorial per fare mappe alimentate da google spreadsheet = \ |LINK106|\  
...
.. _h41a1d42346d60703c5f4a5910454b47:

\ |LINK107|\  e \ |LINK108|\  UMAP AVVISI POLIZIA MUNICIPALE PA - VISUALIZZARE MAPPE CON INFO SU DATA ODIERNA
-------------------------------------------------------------------------------------------------------------
...
⇒ \ |LINK109|\  plug in come componente aggiuntivo da installare su spreadsheet di Google. Sfrutta API di Google e quindi i dati delle coordinate geografiche sono riusabili solo su mappe Google.
...
dove A2 è la colonna dove è contenuto l'indirizzo. \ |LINK110|\ .


|REPLACE10|

* \ |LINK111|\  con Openstreetmap tramite OnData
...
* \ |LINK115|\  con Googlemap

* \ |LINK116|\  by GBVitrano con API Google

* \ |LINK117|\  by GBVitrano con API Google

* \ |LINK118|\  con le API di Google

    * \ |LINK119|\  con API Google

* \ |LINK120|\  (google e bing) 

* \ |LINK121|\  con API di Mapquest

* \ |LINK122|\  API Mapquest per geocoding (cirospat - caneclaudia2volte)

* \ |LINK123|\  location helper in versione demo di Mapbox
...
* \ |LINK125|\  (mappa con grafici e dati su sensore mio)
...
just a simple guide for getting started with git. \ |LINK126|\ 
...
\ |LINK129|\  un articolo per capire un po

\ |LINK130|\  corso online strutturato bene per utenti con nozioni zero di intelligenza artificiale
...
* \ |LINK132|\  editor online sulla falsa riga di Google spreadsheet
...
* \ |LINK139|\  il mio genratore di feed RSS

* \ |LINK140|\  - \ |LINK141|\  per costruire Feed per gli eventi Feltrinelli di Palermo
...
* PDFTK Builder \ |LINK146|\  

* Online2PDF \ |LINK147|\  

* \ |LINK148|\  — servizio Web gratuito e che non richiede registrazioni. Permette di unire e modificare i file PDF in vari modi. Da notare che non consente di caricare file aventi un peso superiore ai 100 MB. Eventualmente, però, è possibile aggirare questa limitazione passando a uno dei piani a pagamento (con costi a partire da 6 euro/mese), i quali consentono di sbloccare anche altre funzionalità extra.

* \ |LINK149|\  — si tratta di un altro servizio Web gratuito e che non necessita di registrazione e permette di intervenire sui documenti PDF unendoli, oltre che convertendoli, dividendoli, alleggerendoli, sbloccandoli ecc. Non applica limitazioni per quel che concerne il peso massimo dei file.

* \ |LINK150|\  — è un’altra soluzione per unire i file PDF via Web e tramite la quale è eventualmente possibile apportare varie ulteriori modifiche ai documenti caricati. È gratis, non richiede registrazione e non pone limiti per l’upload.

\ |STYLE33|\ 

\ |STYLE34|\ . Si tratta, infatti, di un’app che consente di visualizzare i file PDF e intervenire su di essi in vari modi, andandoli a convertire, unire ecc. È gratuita e facile da usare. Per effettuarne il download sul tuo dispositivo, accedi alla \ |LINK151|\ 
...
:guilabel:`Queste ed altre indicazioni per Read the Docs si tovano anche a` \ |LINK153|\  
...
:download:\`Questo link è per il download <\ |LINK154|\ >\`.

si scrive così: ``:download:`Questo link è per il download <\ |LINK155|\ >```
