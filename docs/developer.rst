...
:guilabel:`trovare la pagina esatta in un pdf online`: nel web è \ |LINK10|\  cioè dopo :class:`.pdf` digitare :class:`.#page=4` dove dopo :class:`=` si scrive il numero della pagina che si vuole venga visualizzata direttamente all'apertura del file ``pdf`` online
...
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|icone material design Google                     |\ |LINK11|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|webservice per la trascrizione testuale di audio |\ |LINK12|\                                                                               |
|                                                 |                                                                                          |
|con plugin per chrome                            |\ |LINK13|\                                                                               |
|                                                 |                                                                                          |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|\ |STYLE0|\  funzioni su Fogli Google spreadsheet|\ |LINK14|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|Markup Validation Service                        |\ |LINK15|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|Web accessibility Contrast Checker               |\ |LINK16|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|Web Developer Toolbar Chrome                     |\ |LINK17|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|Accessibility of a PDF Document                  |\ |LINK18|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
...
* \ |LINK21|\  my google maps

* \ |LINK22|\  (pagina FB con mappe su Palermo)

* \ |LINK23|\  (mappe varie di Ciro)

* \ |LINK24|\  (contenitore di oltre 100 mappe su Palermo, su Petrusino)

* \ |LINK25|\  (sito mappe di Palermo utili a turisti)

* \ |LINK26|\  (OMIRA su Umap cirospat) 

* \ |LINK27|\  elezioni2017_Palermo da Guenter Richter

* \ |LINK28|\  e relativo bot telegram #rifiutocivico

* \ |LINK29|\  mapillary cirospat

* \ |LINK30|\  Palermo su mapillary

* \ |LINK31|\  Palermo airbnb gen_2018 (Gianni Vitrano)
...
* \ |LINK32|\  (GBVitrano Palermo IGM 1:25000)

* \ |LINK33|\  carta tecnica comune Palermo

* \ |LINK34|\  (GBVitrano Omira 1:5000)

* \ |LINK35|\   PRG2025 (su Petrusino)

* \ |LINK36|\  (elenco mappe 2-3D di GBVitrano su Github SiciliaHub)

* \ |LINK37|\   (Hubsicilia - Omira Irta e Sas con OSM e RealVista su Gitub SiciliaHub, con metadati mappa su barra laterale)

* \ |LINK38|\  (variante general eal PRG 2004 con cerchio)

* \ |LINK39|\  (catasto centro storico 1887 con cerchio)

* \ |LINK40|\  (PRG scehma masima su PRG 2004 con cerchio)

* \ |LINK41|\    \ |LINK42|\    (Piano protezione civile Palermo)

* \ |LINK43|\   (stile petrusino)

* \ |LINK44|\   (stile petrusino, ultimo gen 2018) - hub di diverse mappe di Palermo a cura di GBVitrano iniziato gli ultimi giorni di dic. del 2017
...
\ |LINK45|\  (che preferisco) 

\ |LINK46|\  (intramontabile) 

\ |LINK47|\  (questo però è un plugin per wordpress) 
...
The \ |LINK51|\  function converts it to the required format for the Query formula by specifying a format of "yyyy-mm-dd":
...
* lista di tiles per sfondi   \ |LINK53|\  

* tiles vari da usare  \ |LINK54|\  

* satellite tile ESRI - World Imagery \ |LINK55|\  

* satellite tile Google \ |LINK56|\  

* tile con sfondo satellite da mapbox/satellite-v9 con API: \ |LINK57|\   (senza nomi strade)

* \ |LINK58|\  (con nomi strade)

* SERVIZIO ONLINE CREAZIONE DI TILES DA IMMAGINI  Mapwarper \ |LINK59|\  

* \ |LINK60|\  tile: https://api.mapbox.com/styles/v1/rplln/cjix2cv5l83yz2rmne9icvusx/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoicnBsbG4iLCJhIjoiY2ppeDI5ZTByMDVtMzN1cDRxZWlzamwzcyJ9.Yn6OjT0SStL2tffNloZrdg
...
video embedding = {{{\ |LINK61|\ }}}

link con testo = [[\ |LINK62|\ |testo del link]] 

immagine = {{\ |LINK63|\ |larghezza}} Immagine 

link a url con icona cliccabile = [[\ |LINK64|\ }}]]

ICONE MAPPE = \ |LINK65|\  

GOOGLEMAPS = <iframe width='100%' height='500' src="http....   "></iframe>

URL ENCODER E DECODER = \ |LINK66|\  

POST IMAGE DA LINKARE SU UMAP: \ |LINK67|\  

Icone mappe \ |LINK68|\  ccbysa
...
Video breve per alimentare in modo facile uMap da un foglio elettronico su Google Drive, by Andrea Borruso \ |LINK69|\ 

Tutorial per fare mappe alimentate da google spreadsheet = \ |LINK70|\  
...
.. _h4b3599535876de1745d605c7d15:

\ |LINK71|\  e \ |LINK72|\  UMAP AVVISI POLIZIA MUNICIPALE PA - VISUALIZZARE MAPPE CON INFO SU DATA ODIERNA
-----------------------------------------------------------------------------------------------------------
...
⇒ \ |LINK73|\  plug in come componente aggiuntivo da installare su spreadsheet di Google. Sfrutta API di Google e quindi i dati delle coordinate geografiche sono riusabili solo su mappe Google.
...
dove A2 è la colonna dove è contenuto l'indirizzo. \ |LINK74|\ .


|REPLACE6|

* \ |LINK75|\  con Openstreetmap tramite OnData
...
* \ |LINK79|\  con Googlemap

* \ |LINK80|\  by GBVitrano con API Google

* \ |LINK81|\  by GBVitrano con API Google

* \ |LINK82|\  con le API di Google

    * \ |LINK83|\  con API Google

* \ |LINK84|\  (google e bing) 

* \ |LINK85|\  con API di Mapquest

* \ |LINK86|\  API Mapquest per geocoding (cirospat - caneclaudia2volte)

* \ |LINK87|\  location helper in versione demo di Mapbox
...
* \ |LINK89|\  (mappa con grafici e dati su sensore mio)
...
just a simple guide for getting started with git. \ |LINK90|\ 
...
\ |LINK93|\  un articolo per capire un po

\ |LINK94|\  corso online strutturato bene per utenti con nozioni zero di intelligenza artificiale
...
* \ |LINK96|\  editor online sulla falsa riga di Google spreadsheet
...
* \ |LINK103|\  il mio genratore di feed RSS

* \ |LINK104|\  - \ |LINK105|\  per costruire Feed per gli eventi Feltrinelli di Palermo
...
* PDFTK Builder \ |LINK110|\  

* Online2PDF \ |LINK111|\  

* \ |LINK112|\  — servizio Web gratuito e che non richiede registrazioni. Permette di unire e modificare i file PDF in vari modi. Da notare che non consente di caricare file aventi un peso superiore ai 100 MB. Eventualmente, però, è possibile aggirare questa limitazione passando a uno dei piani a pagamento (con costi a partire da 6 euro/mese), i quali consentono di sbloccare anche altre funzionalità extra.

* \ |LINK113|\  — si tratta di un altro servizio Web gratuito e che non necessita di registrazione e permette di intervenire sui documenti PDF unendoli, oltre che convertendoli, dividendoli, alleggerendoli, sbloccandoli ecc. Non applica limitazioni per quel che concerne il peso massimo dei file.

* \ |LINK114|\  — è un’altra soluzione per unire i file PDF via Web e tramite la quale è eventualmente possibile apportare varie ulteriori modifiche ai documenti caricati. È gratis, non richiede registrazione e non pone limiti per l’upload.

\ |STYLE10|\ 

\ |STYLE11|\ . Si tratta, infatti, di un’app che consente di visualizzare i file PDF e intervenire su di essi in vari modi, andandoli a convertire, unire ecc. È gratuita e facile da usare. Per effettuarne il download sul tuo dispositivo, accedi alla \ |LINK115|\ 
...
:guilabel:`Queste ed altre indicazioni per Read the Docs si tovano anche a` \ |LINK117|\  
...
:download:\`Questo link è per il download <\ |LINK118|\ >\`.

si scrive così: ``:download:`Questo link è per il download <\ |LINK119|\ >```
