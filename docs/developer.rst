...
* Associazione OnData: \ |LINK4|\ 

* Riccardo Borchi: \ |LINK5|\ 

* Governo: \ |LINK6|\ 

* Bot Telegram COVID-19 Italia Aggiornamento: \ |LINK7|\  
...
:guilabel:`trovare la pagina esatta in un pdf online`: nel web è \ |LINK12|\  cioè dopo :class:`.pdf` digitare :class:`.#page=4` dove dopo :class:`=` si scrive il numero della pagina che si vuole venga visualizzata direttamente all'apertura del file ``pdf`` online
...
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|icone material design Google                     |\ |LINK13|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|webservice per la trascrizione testuale di audio |\ |LINK14|\                                                                               |
|                                                 |                                                                                          |
|con plugin per chrome                            |\ |LINK15|\                                                                               |
|                                                 |                                                                                          |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|\ |STYLE0|\  funzioni su Fogli Google spreadsheet|\ |LINK16|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|Markup Validation Service                        |\ |LINK17|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|Web accessibility Contrast Checker               |\ |LINK18|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|Web Developer Toolbar Chrome                     |\ |LINK19|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
|Accessibility of a PDF Document                  |\ |LINK20|\                                                                               |
+-------------------------------------------------+------------------------------------------------------------------------------------------+
...
* \ |LINK23|\  my google maps

* \ |LINK24|\  (pagina FB con mappe su Palermo)

* \ |LINK25|\  (mappe varie di Ciro)

* \ |LINK26|\  (contenitore di oltre 100 mappe su Palermo, su Petrusino)

* \ |LINK27|\  (sito mappe di Palermo utili a turisti)

* \ |LINK28|\  (OMIRA su Umap cirospat) 

* \ |LINK29|\  elezioni2017_Palermo da Guenter Richter

* \ |LINK30|\  e relativo bot telegram #rifiutocivico

* \ |LINK31|\  mapillary cirospat

* \ |LINK32|\  Palermo su mapillary

* \ |LINK33|\  Palermo airbnb gen_2018 (Gianni Vitrano)
...
* \ |LINK34|\  (GBVitrano Palermo IGM 1:25000)

* \ |LINK35|\  carta tecnica comune Palermo

* \ |LINK36|\  (GBVitrano Omira 1:5000)

* \ |LINK37|\   PRG2025 (su Petrusino)

* \ |LINK38|\  (elenco mappe 2-3D di GBVitrano su Github SiciliaHub)

* \ |LINK39|\   (Hubsicilia - Omira Irta e Sas con OSM e RealVista su Gitub SiciliaHub, con metadati mappa su barra laterale)

* \ |LINK40|\  (variante general eal PRG 2004 con cerchio)

* \ |LINK41|\  (catasto centro storico 1887 con cerchio)

* \ |LINK42|\  (PRG scehma masima su PRG 2004 con cerchio)

* \ |LINK43|\    \ |LINK44|\    (Piano protezione civile Palermo)

* \ |LINK45|\   (stile petrusino)

* \ |LINK46|\   (stile petrusino, ultimo gen 2018) - hub di diverse mappe di Palermo a cura di GBVitrano iniziato gli ultimi giorni di dic. del 2017
...
\ |LINK47|\  (che preferisco) 

\ |LINK48|\  (intramontabile) 

\ |LINK49|\  (questo però è un plugin per wordpress) 
...
The \ |LINK53|\  function converts it to the required format for the Query formula by specifying a format of "yyyy-mm-dd":
...
* lista di tiles per sfondi   \ |LINK55|\  

* tiles vari da usare  \ |LINK56|\  

* satellite tile ESRI - World Imagery \ |LINK57|\  

* satellite tile Google \ |LINK58|\  

* tile con sfondo satellite da mapbox/satellite-v9 con API: \ |LINK59|\   (senza nomi strade)

* \ |LINK60|\  (con nomi strade)

* SERVIZIO ONLINE CREAZIONE DI TILES DA IMMAGINI  Mapwarper \ |LINK61|\  

* \ |LINK62|\  tile: https://api.mapbox.com/styles/v1/rplln/cjix2cv5l83yz2rmne9icvusx/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoicnBsbG4iLCJhIjoiY2ppeDI5ZTByMDVtMzN1cDRxZWlzamwzcyJ9.Yn6OjT0SStL2tffNloZrdg
...
video embedding = {{{\ |LINK63|\ }}}

link con testo = [[\ |LINK64|\ |testo del link]] 

immagine = {{\ |LINK65|\ |larghezza}} Immagine 

link a url con icona cliccabile = [[\ |LINK66|\ }}]]

ICONE MAPPE = \ |LINK67|\  

GOOGLEMAPS = <iframe width='100%' height='500' src="http....   "></iframe>

URL ENCODER E DECODER = \ |LINK68|\  

POST IMAGE DA LINKARE SU UMAP: \ |LINK69|\  

Icone mappe \ |LINK70|\  ccbysa
...
Video breve per alimentare in modo facile uMap da un foglio elettronico su Google Drive, by Andrea Borruso \ |LINK71|\ 

Tutorial per fare mappe alimentate da google spreadsheet = \ |LINK72|\  
...
.. _h213a4675c4d414712266634316f4a17:

\ |LINK73|\  e \ |LINK74|\  UMAP AVVISI POLIZIA MUNICIPALE PA - VISUALIZZARE MAPPE CON INFO SU DATA ODIERNA
-----------------------------------------------------------------------------------------------------------
...
⇒ \ |LINK75|\  plug in come componente aggiuntivo da installare su spreadsheet di Google. Sfrutta API di Google e quindi i dati delle coordinate geografiche sono riusabili solo su mappe Google.
...
dove A2 è la colonna dove è contenuto l'indirizzo. \ |LINK76|\ .


|REPLACE7|

* \ |LINK77|\  con Openstreetmap tramite OnData

* \ |LINK78|\   con Openstreetmap

* \ |LINK79|\  con Openstreetmap

* \ |LINK80|\  con Openstreetmap

* \ |LINK81|\  con Googlemap

* \ |LINK82|\  by GBVitrano con API Google

* \ |LINK83|\  by GBVitrano con API Google

* \ |LINK84|\  con le API di Google

    * \ |LINK85|\  con API Google

* \ |LINK86|\  (google e bing) 

* \ |LINK87|\  con API di Mapquest

* \ |LINK88|\  API Mapquest per geocoding (cirospat - caneclaudia2volte)

* \ |LINK89|\  location helper in versione demo di Mapbox
...
* \ |LINK91|\  (mappa con grafici e dati su sensore mio)
...
just a simple guide for getting started with git. \ |LINK92|\ 
...
\ |LINK95|\  un articolo per capire un po

\ |LINK96|\  corso online strutturato bene per utenti con nozioni zero di intelligenza artificiale
...
* \ |LINK98|\  editor online sulla falsa riga di Google spreadsheet
...
* \ |LINK105|\  il mio genratore di feed RSS

* \ |LINK106|\  - \ |LINK107|\  per costruire Feed per gli eventi Feltrinelli di Palermo
...
* PDFTK Builder \ |LINK112|\  

* Online2PDF \ |LINK113|\  

* \ |LINK114|\  — servizio Web gratuito e che non richiede registrazioni. Permette di unire e modificare i file PDF in vari modi. Da notare che non consente di caricare file aventi un peso superiore ai 100 MB. Eventualmente, però, è possibile aggirare questa limitazione passando a uno dei piani a pagamento (con costi a partire da 6 euro/mese), i quali consentono di sbloccare anche altre funzionalità extra.

* \ |LINK115|\  — si tratta di un altro servizio Web gratuito e che non necessita di registrazione e permette di intervenire sui documenti PDF unendoli, oltre che convertendoli, dividendoli, alleggerendoli, sbloccandoli ecc. Non applica limitazioni per quel che concerne il peso massimo dei file.

* \ |LINK116|\  — è un’altra soluzione per unire i file PDF via Web e tramite la quale è eventualmente possibile apportare varie ulteriori modifiche ai documenti caricati. È gratis, non richiede registrazione e non pone limiti per l’upload.

\ |STYLE10|\ 

\ |STYLE11|\ . Si tratta, infatti, di un’app che consente di visualizzare i file PDF e intervenire su di essi in vari modi, andandoli a convertire, unire ecc. È gratuita e facile da usare. Per effettuarne il download sul tuo dispositivo, accedi alla \ |LINK117|\ 
...
:guilabel:`Queste ed altre indicazioni per Read the Docs si tovano anche a` \ |LINK119|\  
...
:download:\`Questo link è per il download <\ |LINK120|\ >\`.

si scrive così: ``:download:`Questo link è per il download <\ |LINK121|\ >```
